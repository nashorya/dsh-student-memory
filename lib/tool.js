const WRITE_LESSON_DESCRIPTION = "Record a lesson after a wrong-then-right correction. Use an arcId from Open arcs in the current runtime context. Fill only semantic fields.";
function asDraft(args) {
  const rec = args && typeof args === "object" ? args : {};
  const str = (key) => typeof rec[key] === "string" ? rec[key] : "";
  return {
    arcId: str("arcId"),
    cause: str("cause"),
...
function planStepTool(getRuntime) {
  return {
    name: "plan_step",
    ...
    async execute(args) {
      ...
      const todos = raw.flatMap((row) => {
        ...
        if (!content) return [];
        const status = item.status === "doing" || item.status === "done" || item.status === "pending" ? item.status : void 0;
...
function writeLessonTool(getRuntime) {
  return {
    name: "write_lesson",
...
function proposeAdrTool(getRuntime) {
  return {
    name: "propose_adr",
...
function updateIndexTool(getRuntime) {
  return {
    name: "update_index",
...
function appendBuglogTool(getRuntime) {
  return {
    name: "append_buglog",
...
function observationFromExec(exec, result) {
  return {
    ...
    resultText: typeof result?.content === "string" ? result.content : JSON.stringify(result?.content ?? result?.error ?? "")
  };
}
export {
